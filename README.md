...
- 🎯 **AI 驱动的学习计划**：输入主题或上传材料，自动生成个性化学习路径
- 📚 **智能讲义生成**：流式生成深度讲义，支持 Markdown、LaTeX 公式、Mermaid 图表
- 💬 **聊天式追问**：右侧栏实时对话，不懂就问，可拖动调整宽度
- 📝 **多题型测验**：单选、多选、判断、填空、简答、编程题全覆盖
- ❌ **智能错题本**：自动记录错题，针对性复习
- 🔄 **FSRS 复习系统**：科学间隔复习，长期记忆更牢固
- 📊 **可视化统计**：复习热力图、学习曲线、保留率分析
- 📂 **完整文件上传**：支持 PDF、Word、PPT、图片（自动理解）、TXT、Markdown
- 🎨 **精美界面**：深色模式、字号调节、专注模式、高亮标记
- ⚙️ **开发友好**：debug 模式支持自定义 API 站点（生产环境锁定）
...
如需在本地使用自定义 API 配置，查看 [DEV_CONFIG.md](./DEV_CONFIG.md)
...
点击「新建课程」按钮，选择：
- **输入主题**：直接输入学习主题（如「React 进阶」）
- **上传材料**：拖拽上传 PDF、Word、PPT、图片等文件，AI 自动提取内容

设置学习节奏（冲刺/常规/休闲）、总天数和每天学习时长，AI 会自动生成完整的学习计划。
