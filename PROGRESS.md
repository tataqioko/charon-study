...
7. [x] 材料上传功能（完整实现）
   ...
   - [x] TXT/MD 文本文件支持
   - [x] PDF 支持（pdfjs-dist 前端解析）
   - [x] Word 支持（.docx，mammoth.js）
   - [x] PPT 支持（.pptx，pizzip XML 解析）
   - [x] 图片支持（调用视觉模型直接理解）
   - [x] 图片自动压缩（最大边 2048px）
   - [x] 多文件合并 → knowledge_base 存库 + AI 生成课程
   - [x] 文件大小上限 10MB
8. [x] 追问功能改造:右侧栏聊天窗口式显示
   - [x] 创建 qa 表存储问答历史
   - [x] 右侧栏聊天气泡样式（用户/AI 颜色区分）
   - [x] 追问不再追加到讲义,独立存储和显示
   - [x] 切换讲义时加载对应问答历史
   - [x] Markdown 渲染问答内容
   - [x] 聊天式布局优化（输入框在底部）
   - [x] 右侧栏可拖拽调整宽度（240px-800px）
   - [x] 字号与讲义同步
   - [x] 移除笔记侧栏，保留高亮功能
9. [x] FSRS 复习系统 + 可视化
...
1. [ ] 测验系统日历选择
   - [ ] 课程列表点击测验显示日历弹窗
   - [ ] 日历显示每天测验状态（可用/已完成/未解锁）
   - [ ] 点击日期打开对应测验
2. [ ] AI 生成进度实时提示
   - [ ] 右上角持续性浮窗显示生成进度
   - [ ] 详细显示当前阶段（规划/生成讲义/生成测验）
   - [ ] 进度百分比 + 预计剩余时间
3. [ ] UI 布局优化
   - [ ] 左侧栏设置按钮移到左下角
   - [ ] 侧栏折叠/展开动画（motion-v）
