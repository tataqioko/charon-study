...
<template>
  <div class="max-w-4xl mx-auto px-8 py-8">
    ...
    >
      <div>
        <h1 class="text-2xl font-bold tracking-tight">测验系统</h1>
        <p class="text-sm text-muted-foreground mt-1">选择课程进入测验</p>
      </div>
      <button
        @click="router.push('/app/wrong-answers')"
        class="px-4 py-2 rounded-md border border-border hover:bg-accent transition-colors flex items-center gap-2 text-sm"
      >
    ...
    <div v-else-if="!courses.length" class="py-24 flex flex-col items-center gap-3 text-center text-muted-foreground">
      <Icon name="solar:clipboard-check-bold-duotone" class="size-14 text-primary/40" />
      <p class="text-sm">还没有课程,先创建课程后才能测验</p>
    </div>
    ...
    <div v-else class="grid grid-cols-2 gap-4">
      ...
      >
        <Card class="cursor-pointer transition-shadow hover:shadow-lg" @click="router.push(`/app/study/${c.id}`)">
          <CardHeader>
