  ...
  <div class="max-w-4xl mx-auto px-8 py-8">
    ...
    >
      <div>
        ...
        <p class="text-sm text-muted-foreground mt-1">查看所有课程的错题记录</p>
      </div>
    </motion.div>
    ...
    <div v-else-if="!courses.length" class="py-24 flex flex-col items-center gap-3 text-center text-muted-foreground">
      <Icon name="solar:document-text-bold-duotone" class="size-14 text-primary/40" />
      <p class="text-sm">还没有课程,先创建课程后才能查看错题</p>
    </div>
    ...
    <div v-else class="grid grid-cols-2 gap-4">
      ...
      >
        <Card class="cursor-pointer transition-shadow hover:shadow-lg" @click="router.push(`/app/wrong-answers/${c.id}`)">
          <CardHeader>
